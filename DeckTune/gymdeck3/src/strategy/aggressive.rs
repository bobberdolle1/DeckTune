//! Aggressive adaptation strategy with fast adaptation (500ms ramp)

use super::{AdaptationStrategy, CoreBounds, clamp_to_bounds, lerp};

/// Aggressive strategy with 500ms ramp time
///
/// This strategy provides fast adaptation to load changes,
/// prioritizing responsiveness over stability. Ideal for users
/// who want quick reactions to workload changes.
...
impl AdaptationStrategy for AggressiveStrategy {
...
mod tests {
    ...
    #[test]
    fn test_aggressive_respects_bounds() {
        let strategy = AggressiveStrategy::new();
        let bounds = CoreBounds {
            min_mv: -20,
            max_mv: -35,
