...
$TEMP_DIR = Join-Path $RELEASE_DIR "temp"
...
Get-ChildItem $TEMP_DIR -Recurse -Force | Where-Object {
    $_.Name -like "._*" -or $_.Name -eq ".DS_Store" -or $_.Name -eq "__pycache__"
} | Remove-Item -Recurse -Force

# Create zip (files in root, not in DeckTune folder)
Write-Host ""
Write-Host "5. Creating zip archive..." -ForegroundColor Yellow
Set-Location $TEMP_DIR
Compress-Archive -Path "*" -DestinationPath (Join-Path $RELEASE_DIR "DeckTune.zip") -Force
